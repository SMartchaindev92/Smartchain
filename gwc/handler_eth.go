...
// ethHandler implements the gwc.Backend interface to handle the various network
// packets that are sent as replies or broadcasts.
type ethHandler handler

func (h *ethHandler) Chain() *core.BlockChain     { return h.chain }
func (h *ethHandler) StateBloom() *trie.SyncBloom { return h.stateBloom }
func (h *ethHandler) TxPool() gwc.TxPool          { return h.txpool }

// RunPeer is invoked when a peer joins on the `gwc` protocol.
func (h *ethHandler) RunPeer(peer *gwc.Peer, hand gwc.Handler) error {
	return (*handler)(h).runEthPeer(peer, hand)
}

// PeerInfo retrieves all known `gwc` information about a peer.
...
func (h *ethHandler) Handle(peer *gwc.Peer, packet gwc.Packet) error {
	...
	default:
		return fmt.Errorf("unexpected gwc packet type: %T", packet)
	}
}

// handleHeaders is invoked from a peer's message handler when it transmits a batch
// of headers for the local node to process.
func (h *ethHandler) handleHeaders(peer *gwc.Peer, headers []*types.Header) error {
...
func (h *ethHandler) handleBodies(peer *gwc.Peer, txs [][]*types.Transaction, uncles [][]*types.Header) error {
...
func (h *ethHandler) handleBlockAnnounces(peer *gwc.Peer, hashes []common.Hash, numbers []uint64) error {
...
func (h *ethHandler) handleBlockBroadcast(peer *gwc.Peer, block *types.Block, td *big.Int) error {
