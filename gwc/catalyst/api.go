...
import (
	...
	chainParams "github.com/SMartchaindev92/Smartchain/params"
	"github.com/SMartchaindev92/Smartchain/rpc"
	"github.com/SMartchaindev92/Smartchain/trie"
)

// Register adds catalyst APIs to the node.
func Register(stack *node.Node, backend *gwc.Gquantumchain) error {
...
type consensusAPI struct {
	gwc *gwc.Gquantumchain
}

func newConsensusAPI(gwc *gwc.Gquantumchain) *consensusAPI {
	return &consensusAPI{gwc: gwc}
}
...
func (api *consensusAPI) makeEnv(parent *types.Block, header *types.Header) (*blockExecutionEnv, error) {
	state, err := api.gwc.BlockChain().StateAt(parent.Root())
	if err != nil {
		return nil, err
	}
	env := &blockExecutionEnv{
		chain:   api.gwc.BlockChain(),
		state:   state,
		header:  header,
...
func (api *consensusAPI) AssembleBlock(params assembleBlockParams) (*executableData, error) {
	...
	coinbase, err := api.gwc.Etherbase()
	if err != nil {
		return nil, err
	}
	num := parent.Number()
	...
	if config := api.gwc.BlockChain().Config(); config.IsLondon(header.Number) {
		header.BaseFee = misc.CalcBaseFee(config, parent.Header())
	}
	err = api.gwc.Engine().Prepare(bc, header)
	if err != nil {
		return nil, err
	}
	...
	// Create the block.
	block, err := api.gwc.Engine().FinalizeAndAssemble(bc, header, env.state, transactions, nil /* uncles */, env.receipts)
	if err != nil {
		return nil, err
	}
...
func (api *consensusAPI) NewBlock(params executableData) (*newBlockResponse, error) {
	parent := api.gwc.BlockChain().GetBlockByHash(params.ParentHash)
	if parent == nil {
		return &newBlockResponse{false}, fmt.Errorf("could not find parent %x", params.ParentHash)
	}
	block, err := insertBlockParamsToBlock(api.gwc.BlockChain().Config(), parent.Header(), params)
	if err != nil {
		return nil, err
	}
	_, err = api.gwc.BlockChain().InsertChainWithoutSealVerification(block)
