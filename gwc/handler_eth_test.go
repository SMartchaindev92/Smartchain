...
import (
	...
	"github.com/SMartchaindev92/Smartchain/consensus/ethash"
...
// on the `gwc` protocol and convert them into a more easily testable form.
...
func (h *testEthHandler) TxPool() gwc.TxPool                   { panic("no backing tx pool") }
func (h *testEthHandler) AcceptTxs() bool                      { return true }
func (h *testEthHandler) RunPeer(*gwc.Peer, gwc.Handler) error { panic("not used in tests") }
func (h *testEthHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testEthHandler) Handle(peer *gwc.Peer, packet gwc.Packet) error {
	switch packet := packet.(type) {
	case *gwc.NewBlockPacket:
		h.blockBroadcasts.Send(packet.Block)
		return nil

	case *gwc.NewPooledTransactionHashesPacket:
		h.txAnnounces.Send(([]common.Hash)(*packet))
		return nil

	case *gwc.TransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	case *gwc.PooledTransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	default:
		panic(fmt.Sprintf("unexpected gwc packet type in tests: %T", packet))
	}
}

// Tests that peers are correctly accepted (or rejected) based on the advertised
// fork IDs in the protocol handshake.
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, gwc.ETH66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	peerNoFork := gwc.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := gwc.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- ethNoFork.runEthPeer(peerProFork, func(peer *gwc.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- ethProFork.runEthPeer(peerNoFork, func(peer *gwc.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = gwc.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = gwc.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, gwc.ETH66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := gwc.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := gwc.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runEthPeer(sink, func(peer *gwc.Peer) error {
		return gwc.Handle((*ethHandler)(handler.handler), peer)
	})
...
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, gwc.ETH66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runEthPeer(src, func(peer *gwc.Peer) error {
		return gwc.Handle((*ethHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 65, 66:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post gwc/65")
			}
...
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, gwc.ETH66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := gwc.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := gwc.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runEthPeer(sourcePeer, func(peer *gwc.Peer) error {
			return gwc.Handle((*ethHandler)(source.handler), peer)
		})
		go sink.handler.runEthPeer(sinkPeer, func(peer *gwc.Peer) error {
			return gwc.Handle((*ethHandler)(sink.handler), peer)
		})
	}
...
// Tests that post gwc protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := gwc.NewPeer(gwc.ETH66, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := gwc.NewPeer(gwc.ETH66, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runEthPeer(local, func(peer *gwc.Peer) error {
			return gwc.Handle((*ethHandler)(handler.handler), peer)
		})
	}()
	...
	if checkpoint {
		...
		request := new(gwc.GetBlockHeadersPacket66)
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := gwc.NewPeer(gwc.ETH66, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := gwc.NewPeer(gwc.ETH66, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, gwc.ETH66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := gwc.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := gwc.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runEthPeer(src, func(peer *gwc.Peer) error {
		return gwc.Handle((*ethHandler)(source.handler), peer)
	})
