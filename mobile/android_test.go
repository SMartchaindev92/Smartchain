...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gwc-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/SMartchaindev92/Smartchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gwc.aar"), "gwc.aar")
