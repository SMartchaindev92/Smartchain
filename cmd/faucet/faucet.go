...
import (
	...
	"github.com/SMartchaindev92/Smartchain/cmd/utils"
	...
	"github.com/SMartchaindev92/Smartchain/ethclient"
...
var (
	...
	ethPortFlag = flag.Int("ethport", 30304, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Gquantumchain protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Gquantumchain and the faucet")
...
type request struct {
	...
	Account common.Address     `json:"account"` // Gquantumchain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Gquantumchain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Gquantumchain protocol stack
	client *ethclient.Client   // Client connection to the Gquantumchain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Gquantumchain light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Gquantumchain service: %w", err)
	}
...
// close terminates the Gquantumchain connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Gquantumchain address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/SMartchaindev92/Smartchain/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
...
// the uniqueness identifier (user id/username), username, avatar URL and Gquantumchain address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Gquantumchain address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Gquantumchain address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Gquantumchain address to fund on
// success.
...
// returning the username, avatar URL and Gquantumchain address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Gquantumchain address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Gquantumchain address,
