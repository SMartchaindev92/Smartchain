...
import (
	...
	"github.com/SMartchaindev92/Smartchain/accounts/keystore"
	"github.com/SMartchaindev92/Smartchain/common"
	"github.com/SMartchaindev92/Smartchain/common/fdlimit"
	...
	"github.com/SMartchaindev92/Smartchain/crypto"
	"github.com/SMartchaindev92/Smartchain/gwc"
	...
	"github.com/SMartchaindev92/Smartchain/gwc/tracers"
	"github.com/SMartchaindev92/Smartchain/ethdb"
	"github.com/SMartchaindev92/Smartchain/ethstats"
	"github.com/SMartchaindev92/Smartchain/graphql"
	"github.com/SMartchaindev92/Smartchain/internal/ethapi"
	"github.com/SMartchaindev92/Smartchain/internal/flags"
	"github.com/SMartchaindev92/Smartchain/les"
	"github.com/SMartchaindev92/Smartchain/log"
	"github.com/SMartchaindev92/Smartchain/metrics"
	"github.com/SMartchaindev92/Smartchain/metrics/exp"
	"github.com/SMartchaindev92/Smartchain/metrics/influxdb"
	...
	"github.com/SMartchaindev92/Smartchain/p2p/nat"
	"github.com/SMartchaindev92/Smartchain/p2p/netutil"
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gwc account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "GQC", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
// SetEthConfig applies gwc-related command line flags to the config.
...
// RegisterEthService adds an Gquantumchain client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *gwc.Gquantumchain) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Gquantumchain service: %v", err)
		}
		stack.RegisterAPIs(tracers.APIs(backend.ApiBackend))
		return backend.ApiBackend, nil
	}
	backend, err := gwc.New(stack, cfg)
	if err != nil {
		Fatalf("Failed to register the Gquantumchain service: %v", err)
	}
	if cfg.LightServ > 0 {
...
// RegisterEthStatsService configures the Gquantumchain Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Gquantumchain Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gwc.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.GlobalString(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gwc.", tagsMap)
		}
...
// new format.
//
// e.g. gwc account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gwc --keystore /tmp/mykeystore --lightkdf account new
//
