...
// nodeDockerfile is the Dockerfile required to run an Gquantumchain node.
var nodeDockerfile = `
...
{{end}}
RUN \
  echo 'gwc --cache 512 init /genesis.json' > gwc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.gwc/keystore/ && cp /signer.json /root/.gwc/keystore/' >> gwc.sh && \{{end}}
	echo $'exec gwc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gwc.sh

ENTRYPOINT ["/bin/sh", "gwc.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Gquantumchain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Gquantumchain node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gwc --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
