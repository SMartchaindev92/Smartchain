...
// Start gwc with
//
// build/bin/gwc --nodiscover --maxpeers 0 --signer http://localhost:8550 console --preload=cmd/clef/tests/testsigner.js
//
...
function testTx(){
    if( accts && accts.length > 0) {
        var a = accts[0]
        var txdata = gwc.signTransaction({from: a, to: a, value: 1, nonce: 1, gas: 1, gasPrice: 1})
...
function testSignText(){
    if( accts && accts.length > 0){
        var a = accts[0]
        var r = gwc.sign(a, "0x68656c6c6f20776f726c64"); //hello world
