...
func (g *gethrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gwc.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gethrpc) addPeer(peer *gethrpc) {
	g.gwc.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gwc.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gwc.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gwc.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gwc.Error("timeout adding peer after", dur)
	}
}
...
func (g *gethrpc) waitSynced() {
	...
	if ok && !syncing {
		g.gwc.Logf("%v already synced", g.name)
		return
	}

	// Actually wait, subscribe to the event
	ch := make(chan interface{})
	sub, err := g.rpc.Subscribe(context.Background(), "gwc", ch, "syncing")
	if err != nil {
		g.gwc.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gwc.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gwc.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gwc.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gwc.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGethWithIpc(t *testing.T, name string, args ...string) *gethrpc {
	ipcName := fmt.Sprintf("gwc-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	ipcpath := ipcEndpoint(ipcName, g.gwc.Datadir)
	// We can't know exactly how long gwc will take to start, so we try 10
...
func initGeth(t *testing.T) string {
	...
	t.Logf("Initializing gwc: %v ", args)
	g := runGeth(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
