...
func TestAccountListEmpty(t *testing.T) {
	gwc := runGeth(t, "account", "list")
	gwc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gwc := runGeth(t, "account", "list", "--datadir", datadir)
	defer gwc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gwc := runGeth(t, "account", "new", "--lightkdf")
	defer gwc.ExpectExit()
	gwc.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gwc := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gwc.ExpectExit()
	gwc.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gwc := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gwc.ExpectExit()
	gwc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gwc.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gwc := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gwc.StderrText(), m) {
...
// https://github.com/SMartchaindev92/Smartchain/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gwc.SetTemplateFunc("keypath", func(file string) string {
