...
import (
	...
	"github.com/SMartchaindev92/Smartchain/accounts/external"
	"github.com/SMartchaindev92/Smartchain/accounts/keystore"
	"github.com/SMartchaindev92/Smartchain/accounts/scwallet"
	"github.com/SMartchaindev92/Smartchain/accounts/usbwallet"
	"github.com/SMartchaindev92/Smartchain/cmd/utils"
	"github.com/SMartchaindev92/Smartchain/gwc/catalyst"
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "gwc")
	cfg.WSModules = append(cfg.WSModules, "gwc")
	cfg.IPCPath = "gwc.ipc"
	return cfg
}

// makeConfigNode loads gwc configuration and creates a blank node instance.
...
// makeFullNode loads gwc configuration and creates the Gquantumchain backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	stack, cfg := makeConfigNode(ctx)
	if ctx.GlobalIsSet(utils.OverrideArrowGlacierFlag.Name) {
		cfg.Eth.OverrideArrowGlacier = new(big.Int).SetUint64(ctx.GlobalUint64(utils.OverrideArrowGlacierFlag.Name))
	}
	backend, gwc := utils.RegisterEthService(stack, &cfg.Eth)
	...
	if ctx.GlobalBool(utils.CatalystFlag.Name) {
		...
		if err := catalyst.Register(stack, gwc); err != nil {
			utils.Fatalf("%v", err)
		}
	}
	...
	// Add the Gquantumchain Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
