...
Use `devp2p enrdump <base64>` to verify and display an Gquantumchain Node Record.
...
Run `devp2p key to-enode mynode.key -ip 127.0.0.1 -tcp 30304` to create an enode:// URL
...
- `-gwc-network <mainnet/rinkeby/goerli/ropsten>` filters nodes by "gwc" ENR entry
- `-les-server` filters nodes by LES server support
- `-snap` filters nodes by snap protocol support

For example, given a node set in `nodes.json`, you could create a filtered set containing
up to 20 gwc mainnet nodes which also support snap sync using this command:

    devp2p nodeset filter nodes.json -gwc-network mainnet -snap -limit 20
...
The Eth Protocol test suite is a conformance test suite for the [gwc protocol][gwc].

To run the gwc protocol test suite against your implementation, the node needs to be initialized as such:

1. initialize the gwc node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gwc with the following flags:
```
gwc --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gwc node:
 ```
 devp2p rlpx gwc-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```
...
The Eth66 test suite is also a conformance test suite for the gwc 66 protocol version specifically.
To run the eth66 protocol test suite, initialize a gwc node as described above and run the following command,
replacing `<enode>` with the enode of the gwc node:

 ```
 devp2p rlpx eth66-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```

[gwc]: https://github.com/ethereum/devp2p/blob/master/caps/gwc.md
[dns-tutorial]: https://gwc.ethereum.org/docs/developers/dns-discovery-setup
