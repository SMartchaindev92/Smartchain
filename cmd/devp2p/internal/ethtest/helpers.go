...
import (
	...
	"github.com/SMartchaindev92/Smartchain/internal/utesting"
	"github.com/SMartchaindev92/Smartchain/p2p"
	"github.com/SMartchaindev92/Smartchain/p2p/rlpx"
)
...
func (s *Suite) dial66() (*Conn, error) {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "gwc", Version: 66})
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("unexpected gwc protocol version")
		}
		return nil
	default:
...
// negotiateEthProtocol sets the Conn's gwc protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "gwc" {
			continue
		}
...
loop:
	...
	// make sure gwc protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("gwc protocol version must be set in Conn")
	}
	if status == nil {
...
func (c *Conn) readAndServe66(chain *Chain, timeout time.Duration) (uint64, Message) {
	...
	for time.Since(start) < timeout {
		...
		case GetBlockHeaders:
			...
			resp := &gwc.BlockHeadersPacket66{
				RequestId:          reqID,
				BlockHeadersPacket: gwc.BlockHeadersPacket(headers),
			}
...
func getBlockHeaders66(chain *Chain, conn *Conn, request *GetBlockHeaders, id uint64) (BlockHeaders, error) {
	// write request
	packet := gwc.GetBlockHeadersPacket(*request)
	req := &gwc.GetBlockHeadersPacket66{
		RequestId:             id,
...
func (s *Suite) hashAnnounce(isEth66 bool) error {
	...
	if isEth66 {
		...
		if err := sendConn.Write66(&gwc.BlockHeadersPacket66{
			RequestId: id,
			BlockHeadersPacket: gwc.BlockHeadersPacket{
				nextBlock.Header(),
			},
