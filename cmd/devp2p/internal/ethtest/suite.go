...
// NewSuite creates and returns a new gwc-test suite that can
...
func (s *Suite) Eth66Tests() []utesting.Test {
	return []utesting.Test{
		// only proceed with eth66 test suite if node supports gwc 66 protocol
		{Name: "TestStatus66", Fn: s.TestStatus66},
		{Name: "TestGetBlockHeaders66", Fn: s.TestGetBlockHeaders66},
		{Name: "TestSimultaneousRequests66", Fn: s.TestSimultaneousRequests66},
		{Name: "TestSameRequestID66", Fn: s.TestSameRequestID66},
...
func (s *Suite) TestSimultaneousRequests66(t *utesting.T) {
	...
	// create two requests
	req1 := &gwc.GetBlockHeadersPacket66{
		RequestId: uint64(111),
		GetBlockHeadersPacket: &gwc.GetBlockHeadersPacket{
			Origin: gwc.HashOrNumber{
				Hash: s.chain.blocks[1].Hash(),
			},
			Amount:  2,
	...
	req2 := &gwc.GetBlockHeadersPacket66{
...
func (s *Suite) TestSameRequestID66(t *utesting.T) {
	...
	// create requests
	reqID := uint64(1234)
	request1 := &gwc.GetBlockHeadersPacket66{
		RequestId: reqID,
		GetBlockHeadersPacket: &gwc.GetBlockHeadersPacket{
			Origin: gwc.HashOrNumber{
				Number: 1,
	...
	request2 := &gwc.GetBlockHeadersPacket66{
		RequestId: reqID,
		GetBlockHeadersPacket: &gwc.GetBlockHeadersPacket{
			Origin: gwc.HashOrNumber{
				Number: 33,
...
func (s *Suite) TestGetBlockBodies66(t *utesting.T) {
	...
	// create block bodies request
	req := &gwc.GetBlockBodiesPacket66{
		RequestId: uint64(55),
		GetBlockBodiesPacket: gwc.GetBlockBodiesPacket{
			s.chain.blocks[54].Hash(),
			s.chain.blocks[75].Hash(),
		},
	}
...
func (s *Suite) TestLargeTxRequest66(t *utesting.T) {
	...
	getTxReq := &gwc.GetPooledTransactionsPacket66{
		RequestId:                   1234,
