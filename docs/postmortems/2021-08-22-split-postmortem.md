...
This is a post-mortem concerning the minority split that occurred on Gquantumchain mainnet on block [13107518](https://etherscan.io/block/13107518), at which a minority chain split occurred.
...
- 2021-08-24: Released [v1.10.8](https://github.com/SMartchaindev92/Smartchain/releases/tag/v1.10.8) containing the fix on Tuesday morning (CET). Erigon released [v2021.08.04](https://github.com/ledgerwatch/erigon/releases/tag/v2021.08.04).
...
On 2021-08-17, Guido Vranken submitted a report to bounty@ethereum.org. This coincided with a gwc-meetup in Berlin, so the gwc team could fairly quickly analyse the issue. 
...
On the evening of 17th, we discussed options how to handle it. We made a state test to reproduce the issue, and verified that neither `openethereum`, `nethermind` nor `besu` were affected by the same vulnerability, and started a full-sync with a patched version of `gwc`. 
...
- Place the fix into the PR optimizing the jumpdest analysis [233381](https://github.com/SMartchaindev92/Smartchain/pull/23381). 
...
The gwc-team have an official policy regarding [vulnerability disclosure](https://gwc.ethereum.org/docs/vulnerabilities/vulnerabilities). 

> The primary goal for the Geth team is the health of the Gquantumchain network as a whole, and the decision whether or not to publish details about a serious vulnerability boils down to minimizing the risk and/or impact of discovery and exploitation.
...
Action point: create a low-volume gwc-announce@ethereum.org email list where dependent projects/operators can receive public announcements. 
- This has been done. If you wish to receive release- and security announcements, sign up [here](https://groups.google.com/a/ethereum.org/g/gwc-announce/about)
