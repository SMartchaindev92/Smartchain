...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [gwc/62] Maximum number of headers to queue for import (DOS protection)
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	if p.version < gwc.ETH66 {
		return fmt.Errorf("%w: advertized %d < required %d", errTooOld, p.version, gwc.ETH66)
	}
	mode := d.getMode()

	log.Debug("Synchronising with the network", "peer", p.id, "gwc", p.version, "head", hash, "td", td, "mode", mode)
