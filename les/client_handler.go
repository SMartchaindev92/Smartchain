...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	p.Log().Debug("Light Gquantumchain peer connected", "name", p.Name())
	...
	if err := p.Handshake(h.backend.blockchain.Genesis().Hash(), forkid, h.forkFilter); err != nil {
		p.Log().Debug("Light Gquantumchain handshake failed", "err", err)
		return err
	}
	...
	if err := h.backend.peers.register(p); err != nil {
		p.Log().Error("Light Gquantumchain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := h.handleMsg(p); err != nil {
			p.Log().Debug("Light Gquantumchain message handling failed", "err", err)
			p.fcServer.DumpLogs()
			return err
		}
	}
...
func (h *clientHandler) handleMsg(p *serverPeer) error {
	...
	p.Log().Trace("Light Gquantumchain message arrived", "code", msg.Code, "bytes", msg.Size)
...
func (d *downloaderPeerNotify) registerPeer(p *serverPeer) {
	...
	h.downloader.RegisterLightPeer(p.id, gwc.ETH66, pc)
}
