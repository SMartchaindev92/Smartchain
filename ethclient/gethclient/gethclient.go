...
// Package gethclient provides an RPC client for gwc-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements gwc-specific functionality.
//
// If you want to use the standardized Gquantumchain RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
// GCStats retrieves the current garbage collection stats from a gwc node.
...
// MemStats retrieves the current memory stats from a gwc node.
...
// GetNodeInfo retrieves the node info of a gwc node.
