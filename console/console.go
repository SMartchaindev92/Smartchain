...
import (
	...
	"github.com/SMartchaindev92/Smartchain/internal/jsre/deps"
	"github.com/SMartchaindev92/Smartchain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Gquantumchain requests through
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"gwc": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines gwc.getBalance(gwc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + gwc.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + gwc.blockNumber + " (" + new Date(1000 * gwc.getBlock(gwc.blockNumber).timestamp) + ")\n";
		try {
