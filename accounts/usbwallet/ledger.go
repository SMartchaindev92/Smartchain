...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-gwc/master/doc/ethapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Gquantumchain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Gquantumchain transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs an Gquantumchain message following the EIP 712 specification
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Gquantumchain app in browser mode", w.failure
	}
	if w.offline() {
		return "Gquantumchain app offline", w.failure
	}
	return fmt.Sprintf("Gquantumchain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the Gquantumchain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Gquantumchain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Gquantumchain app's version, will fail prior to v1.0.2
...
// and returning the Gquantumchain address located on that derivation path.
...
//
// Note, if the version of the Gquantumchain application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Gquantumchain app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Gquantumchain wallet app running
...
// ledgerDerive retrieves the currently active Gquantumchain address from a Ledger
...
//   Gquantumchain address length | 1 byte
//   Gquantumchain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Gquantumchain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Gquantumchain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Gquantumchain signature and do a sanity validation
