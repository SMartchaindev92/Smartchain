...
## Preparing the smartcard
  ...
  **WARNING: FOILLOWING THESE INSTRUCTIONS WILL DESTROY THE MASTER KEY ON YOUR CARD. ONLY PROCEED IF NO FUNDS ARE ASSOCIATED WITH THESE ACCOUNTS**
  ...
  Start `gwc` with the `console` command. You will notice the following warning:

  ```
  WARN [04-09|16:58:38.898] Failed to open wallet                    url=keycard://044def09                          err="smartcard: pairing password needed"
  ```

  Write down the URL (`keycard://044def09` in this example). Then ask `gwc` to open the wallet:
...
## Usage
  ...
  1. Start `gwc` with the `console` command
...
## Known issues
  ...
  * Starting gwc with a valid card seems to make firefox crash.
