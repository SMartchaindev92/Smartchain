...
// PublicEthereumAPI provides an API to access Gquantumchain related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEthereumAPI struct {
	b Backend
}

// NewPublicEthereumAPI creates a new Gquantumchain protocol API.
...
// Sign calculates an Gquantumchain ECDSA signature for:
...
//
// https://github.com/SMartchaindev92/Smartchain/wiki/Management-APIs#personal_sign
...
//
// https://github.com/SMartchaindev92/Smartchain/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Gquantumchain signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gwc.
...
// PublicBlockChainAPI provides an API to access the Gquantumchain blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Gquantumchain blockchain API.
...
// PublicDebugAPI is the collection of Gquantumchain APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Gquantumchain APIs exposed over the private
// debugging endpoint.
