...
// enrEntry is the ENR entry which advertises `gwc` protocol on the discovery.
...
// StartENRUpdater starts the `gwc` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `gwc` ENR entry based on the current state of the chain.
