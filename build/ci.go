...
import (
	...
	"github.com/SMartchaindev92/Smartchain/crypto/signify"
	"github.com/SMartchaindev92/Smartchain/internal/build"
	"github.com/SMartchaindev92/Smartchain/params"
)

var (
	// Files that end up in the gwc*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gwc"),
	}

	// Files that end up in the gwc-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Gquantumchain contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Gquantumchain bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Gquantumchain Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gwc",
			Description: "Gquantumchain CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Gquantumchain private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Gquantumchain account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gwc     = "gwc-" + basegeth + ext
		alltools = "gwc-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gwc, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gwc, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gwc-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gwc-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gwc-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Gquantumchain Builds <fjl@ethereum.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the gwc binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gwc.nsi", filepath.Join(*workdir, "gwc.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gwc-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/SMartchaindev92/Smartchain/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gwc.aar", filepath.Join(GOBIN, "gwc.aar"))
		os.Rename("gwc-sources.jar", filepath.Join(GOBIN, "gwc-sources.jar"))
		return
	}
	...
	archive := "gwc-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gwc.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/SMartchaindev92/Smartchain/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "gwc-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
